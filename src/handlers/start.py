...
async def get_name(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await send_message(chat_id, 'ask-gender', markup=genders_markup)
...
async def get_phone(message: Message, state: FSMContext):
    ...
    await send_message(data["tg_id"], 'ask-phone-number')
    await UserRegistration.next()


@dp.message_handler(state=UserRegistration.gender)
async def get_sweatshirt_size(message: Message, state: FSMContext):
    async with state.proxy() as data:
        data["sweatshirt_size"] = message.text

    await send_message(data["tg_id"], 'ask-sweatshirt-size', markup=sweatshirt_size_markup)
    await UserRegistration.next()
...
async def get_accent_region(message: Message, state: FSMContext):
    ...
    await send_message(data["tg_id"], 'ask-accent-region', markup=ReplyKeyboardRemove())
    await UserRegistration.next()
